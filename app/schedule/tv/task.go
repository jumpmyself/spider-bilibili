...
func GetLink() []string {
	...
	if proxyInfo.IP == "" || proxyInfo.Client == nil {
		logrus.Error("TV没有获取到有效的代理IP或http.Client")
		return nil
	}
	...
	request, err := http.NewRequest("GET", viper.GetString("bilibili_api.tv"), nil)
	if err != nil {
		logrus.Error("请求bilibiliTV失败", err)
	}
	...
	if response.StatusCode != http.StatusOK {
		logrus.Error("请求bilibiliTV失败，状态码：", response.StatusCode)
		return nil
	}
	//这里使用go语言标准库的ioutil.ReadAll函数读取response.body文件。
	body, err := ioutil.ReadAll(response.Body)
	if err != nil {
		logrus.Error("bilibiliTV:读取响应内容失败", err)
		return nil
	}
...
func GetInfo(c *gin.Context) {
	...
	if proxy.IP == "" || proxy.Client == nil {
		...
		c.JSON(http.StatusInternalServerError, tools.ECode{
			Code:    1,
			Message: "TV没有获取到有效的代理IP或http.Client",
		})
		return
	}
	var tv []Tv
	for _, url := range urls {
		...
		if err != nil {
			logrus.Error("TV：http请求失败", err)
		}
		defer response.Body.Close()

		if response.StatusCode != http.StatusOK {
			logrus.Error("TV：http请求失败，状态码：", response.StatusCode)
			return
		}
		body, err := ioutil.ReadAll(response.Body)
		if err != nil {
			logrus.Error("TV：读取响应到body失败", err)
			return
		}
		fmt.Println("body", string(body))
		doc, err := goquery.NewDocumentFromReader(strings.NewReader(string(body)))
		if err != nil {
			logrus.Error("TV将html字符串加载到goquery失败：", err)
		}
