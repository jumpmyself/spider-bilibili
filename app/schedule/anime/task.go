...
func GetLink() []string {
	...
	request, err := http.NewRequest("GET", viper.GetString("bilibili_api.fanju"), nil)
	if err != nil {
		logrus.Error("请求bilibili番剧失败", err)
	}
	...
	if response.StatusCode != http.StatusOK {
		logrus.Error("请求bilibili番剧失败，状态码：", response.StatusCode)
		return nil
	}
...
func GetInfo(c *gin.Context) {
	...
	if proxy.IP == "" || proxy.Client == nil {
		...
		c.JSON(http.StatusInternalServerError, tools.ECode{
			Code:    1,
			Message: "没有获取到有效的代理IP或http.Client",
		})
		return
	}
	...
	// 用于存放解析结果的通道
	animeChan := make(chan Anime, len(urls))
	...
	for _, url := range urls {
		...
		go func(url string) {
			...
			if err != nil {
				logrus.Error("番剧：http请求失败", err)
			}
			defer response.Body.Close()

			if response.StatusCode != http.StatusOK {
				logrus.Error("番剧：http请求失败，状态码：", response.StatusCode)
				return
			}
			body, err := ioutil.ReadAll(response.Body)
			if err != nil {
				logrus.Error("番剧：读取响应到body失败", err)
				return
			}
			...
			doc.Find("#__next > div.home-container > div > div.plp-l.sticky > div > div.mediainfo_mediaInfoWrap__nCwhA > div").Each(func(i int, s *goquery.Selection) {
				...
				a := Anime{
					Title:       title,
					URL:         url,
					Updatedver:  strconv.FormatInt(time.Now().Unix(), 10),
